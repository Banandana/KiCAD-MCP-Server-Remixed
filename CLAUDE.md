...
- **`move_connected`**: Move-with-stretching. Moves a single component by reference, stretches wires connected to its pins, and carries along labels/junctions/power symbols/no-connects at connected points. Steps: (1) get old pin positions via `PinLocator`, (2) trace wires one hop to find far endpoints, (3) move the component via kicad-skip, (4) re-read file, (5) shift wire endpoints at old pin positions to new positions, (6) shift labels/junctions at wire far endpoints, (7) shift power symbols (`lib_id "power:..."`) and no-connects at any connected point (`old_pin_positions | wire_far_endpoints`).
