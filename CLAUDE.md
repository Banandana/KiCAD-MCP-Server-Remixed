...
- **`move_connected`**: Move-with-smart-stretching. Moves a single component by reference, translates stub wires (pin→label) fully, stretches longer wires to other components. Steps: (1) get old pin positions via `PinLocator`, (2) trace wires one hop to find far endpoints, (3) move the component via kicad-skip, (4) re-read file, (5) for stub wires (one end at pin, other at label/junction), translate BOTH endpoints; for other wires, only move the pin-touching endpoint (stretch), (6) shift labels/junctions at wire far endpoints, (7) shift power symbols (`lib_id "power:..."`) and no-connects at any connected point (`old_pin_positions | wire_far_endpoints`).
...
- **`move_connected` uses kicad-skip** for the main component move (step 3), then raw text manipulation for everything else. This is a known fragility point — if kicad-skip fails on the schematic, the whole operation fails. Consider migrating to text-based component move.
...
- **~26 handlers still depend on kicad-skip `SchematicManager.load_schematic()`**: kicad-skip fails on some KiCad 9 schematics. `list_schematic_components` and `PinLocator` have been migrated to regex-based `parse_placed_symbols_from_content()`. Remaining handlers (list_schematic_nets, get_net_connectivity, validate_wire_connections, find_orphan_items, check_schematic_overlaps, get_schematic_layout, annotate_schematic, `move_connected` component move step, etc.) should be migrated as they fail. The regex parser is in `pin_locator.py` and can be imported.
- **`move_connected` uses kicad-skip for the main component move**: Steps 1-2 use `PinLocator` (regex-based, safe), but step 3 uses `SchematicManager.load_schematic()` + kicad-skip to move the component. If kicad-skip fails on the schematic, the entire operation fails. Should be migrated to text-based `(at ...)` replacement like `move_region` uses.
